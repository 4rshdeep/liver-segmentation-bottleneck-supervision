...
def create_weighted_dice(weightMap):

    def weighted_dice(y_true, y_pred , smooth=1):

        y_true_f = keras.flatten(y_true)
        y_pred_f = keras.flatten(y_pred)
        weightMap_f = keras.flatten(weightMap)
        intersection = keras.sum(weightMap_f * (y_true_f*y_pred_f) )
        union = keras.sum(weightMap_f*y_true_f) + keras.sum(weightMap*y_pred_f) + smooth
        dice_coef_f = (2.*intersection + smooth) / (union)
        return 1 - dice_coef_f
...
def denseBlockForBottleneck(input_img,input_channel,growth_rate,output_channel):
...
def weightedSegmentedUnet(input_size=(512,512,1),output_ch=(512,512,1)):
    inputs = Input(input_size)
    weightMap = Input(input_size)
    ...
    model = Model(inputs=[inputs,weightMap], outputs=[convf])

    model.compile(optimizer=Adam(lr=1e-3), loss=create_weighted_dice(weightMap), metrics=[dice_coef])
    
    return model

def bottleneckFeatureWeightedUnet(input_size=(512,512,1),output_ch=(512,512,1),feature_size=(16,16,128)):
    inputs = Input(input_size)
    weightMap = Input(input_size)
    feature_vector_autoencoder = Input(feature_size)
    ...
    concat1 = concatenate([b3,down5],axis=3)
    ...
    convf = Conv2D(output_ch[2],kernel_size=(1,1),padding='same',activation='sigmoid',name="final_output")(up5)

    model = Model(inputs=[inputs,weightMap], outputs=[convf,conv3])

    losses = {
        "final_output" : create_weighted_dice(weightMap),
        "feature_vector" : euclidean_loss,
    }

    lossWeights = { "final_output" : 0.5 , "feature_vector" : 0.5 }

    # model.compile(optimizer=Adam(lr=1e-3), loss=create_weighted_dice(weightMap), metrics=[dice_coef])
    model.compile(optimizer=Adam(lr=1e-3), loss=losses,loss_weights=lossWeights, metrics=[dice_coef])

def bottleneckFeatureUnet(input_size=(512,512,1),output_ch=(512,512,1)):
    inputs = Input(input_size)
    # feature_vector_autoencoder = Input(feature_size)
